      ...
      <div className="flex items-center p-6 border-b border-border/50">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {notes.length > 0 ? `${notes.length} anonymous note${notes.length > 1 ? "s" : ""}` : "No notes"}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 space-y-4">
          ...
          <>
            {notes.map((note) => (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-foreground">
                      {note.role === "sent" ? "Your sent note" : "Someone anonymous"}
                    </span>

                    <span className="ml-2 px-2 text-xs rounded bg-muted text-muted-foreground">
                      {note.role}
                    </span>
                  </div>
                ...
                {/* Content Preview */}
                <p className="text-foreground leading-relaxed mb-4 line-clamp-3">{note.content}</p>
        ...
        {!isPending && !error && notes.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground text-sm">Your anonymous notes will appear here</p>
          </div>
        )}
      </div>
